...
class Controller:
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.stage == 10:
                self.go_to_predefined_pose('rotate_90_1')
                self.rotate(1.57, 1)
                        ...
                        if tomato_num > 0:
                            try:
                                for i in range(tomato_num):
                                    if self.identification_count == 0:
                                        if self.trough_count < 10:
                                            rospy.loginfo("Obj_{} Identified at Trough_no 0{}".format(self.tomato_count, self.trough_count-1))
                                        else:
                                            rospy.loginfo("Obj_{} Identified at Trough_no {}".format(self.tomato_count, self.trough_count-1))
